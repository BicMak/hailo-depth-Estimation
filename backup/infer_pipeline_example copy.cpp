...
static GstFlowReturn on_new_sample(GstElement *sink, gpointer user_data) {
    ...
    // 6. appsrc로 전송
    size_t output_size = depth_resized.total() * depth_resized.elemSize();
    std::cout << "Output buffer size: " << output_size << " (expected: 921600)" << std::endl;
    
    GstBuffer *out_buffer = gst_buffer_new_allocate(NULL, output_size, NULL);
    gst_buffer_fill(out_buffer, 0, depth_resized.data, output_size);

    GST_BUFFER_PTS(out_buffer) = GST_BUFFER_PTS(buffer);
    GST_BUFFER_DTS(out_buffer) = GST_BUFFER_DTS(buffer);
    ...
    std::cout << "appsrc push result: " << ret << " (0=OK)" << std::endl;
...
int main(int argc, char *argv[]) {
    ...
    GstElement *queue_out = gst_element_factory_make("queue", "queue_out");  // 추가!
    ...
    if (!source || !videoconvert1 || !scaler || !queue1 || !hailonet || !appsink ||
        ...
        if (!appsrc) std::cerr << "  - appsrc 실패" << std::endl;
        if (!videoconvert2) std::cerr << "  - videoconvert2 실패" << std::endl;
        if (!sink) std::cerr << "  - sink 실패" << std::endl;
        }

    // 엘리먼트들 파이프라인에 추가
    gst_bin_add_many(GST_BIN(pipeline), 
        source, videoconvert1, scaler, queue1, hailonet, appsink,
        appsrc, queue_out, videoconvert2, sink, NULL);  // queue_out 추가
    ...
    g_object_set(appsrc,
        ...
        "is-live", TRUE,         // ✅ TRUE로 변경
        "do-timestamp", TRUE,
        "min-latency", G_GINT64_CONSTANT(0),
        "max-latency", G_GINT64_CONSTANT(0),
        "block", FALSE,
        NULL);
    g_object_set(queue_out,
        "leaky", 2,           // downstream (오래된 프레임 버림)
        "max-size-buffers", 2,
        "max-size-bytes", 0,
        "max-size-time", 0,
        NULL);
    ...
    // Part 2 연결: appsrc → ... → sink
    GstCaps *caps2 = gst_caps_from_string("video/x-raw,format=BGR,width=640,height=480,framerate=30/1");
    g_object_set(appsrc, "caps", caps2, NULL);
    gst_element_link(appsrc, queue_out);        // appsrc → queue_out
    gst_element_link(queue_out, videoconvert2); // queue_out → videoconvert2
    gst_element_link(videoconvert2, sink);
